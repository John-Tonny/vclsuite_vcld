...
// newAddressPubKey returns a new vclutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) vclutil.Address {
	addr, err := vclutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new vclutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) vclutil.Address {
	addr, err := vclutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new vclutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) vclutil.Address {
	addr, err := vclutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the vclutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the vclutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := vclutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := vclutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := vclutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := vclutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := vclutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*vclutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*vclutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*vclutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*vclutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*vclutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
