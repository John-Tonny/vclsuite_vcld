...
const (
	...
	MinHighPriority = vclutil.SatoshiPerBitcoin * 144.0 / 250
...
func createCoinbaseTx(params *chaincfg.Params, coinbaseScript []byte, nextBlockHeight int32, addr vclutil.Address) (*vclutil.Tx, error) {
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *vclutil.Tx, height int32) error {
...
func logSkippedDeps(tx *vclutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress vclutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*vclutil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		case segwitActive && !witnessIncluded && tx.HasWitness():
			...
			// witness commitment.
			coinbaseCopy := vclutil.NewTx(coinbaseTx.MsgTx().Copy())
...
func (g *BlkTmplGenerator) UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int32, extraNonce uint64) error {
	...
	// TODO(davec): A vclutil.Block should use saved in the state to avoid
