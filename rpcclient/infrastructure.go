...
func (c *Client) BackendVersion() (BackendVersion, error) {
	...
	// to a vcld backend if it does exist.
	info, err := c.GetInfo()

	switch err := err.(type) {
	// Parse the vcld version and cache it.
	case nil:
		log.Debugf("Detected vcld version: %v", info.Version)
		version := Btcd
	...
	case *btcjson.RPCError:
		if err.Code != btcjson.ErrRPCMethodNotFound.Code {
			return 0, fmt.Errorf("unable to detect vcld version: "+
				"%v", err)
		}

	default:
		return 0, fmt.Errorf("unable to detect vcld version: %v", err)
	}
