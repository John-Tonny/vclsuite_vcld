...
const (
	...
	DefaultMinRelayTxFee = vclutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee vclutil.Amount) int64 {
	...
	if minFee < 0 || minFee > vclutil.MaxSatoshi {
		minFee = vclutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *vclutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee vclutil.Amount) bool {
...
func checkTransactionStandard(tx *vclutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee vclutil.Amount,
...
func GetTxVirtualSize(tx *vclutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
