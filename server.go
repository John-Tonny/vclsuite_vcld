...
func (sp *serverPeer) OnTx(_ *peer.Peer, msg *wire.MsgTx) {
	...
	// Convert the raw MsgTx to a vclutil.Tx which provides some convenience
	// methods and things such as hash caching.
	tx := vclutil.NewTx(msg)
...
func (sp *serverPeer) OnBlock(_ *peer.Peer, msg *wire.MsgBlock, buf []byte) {
	// Convert the raw MsgBlock to a vclutil.Block which provides some
	// convenience methods and things such as hash caching.
	block := vclutil.NewBlockFromBlockAndBytes(msg, buf)
...
func (s *server) TransactionConfirmed(tx *vclutil.Tx) {
...
func newServer(listenAddrs, agentBlacklist, agentWhitelist []string,
	...
	txC := mempool.Config{
		...
		CalcSequenceLock: func(tx *vclutil.Tx, view *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error) {
