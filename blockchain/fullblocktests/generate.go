...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: vclutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := vclutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*vclutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, vclutil.NewTx(tx))
	}
...
func additionalCoinbase(amount vclutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee vclutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee vclutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee vclutil.Amount) *wire.MsgTx {
