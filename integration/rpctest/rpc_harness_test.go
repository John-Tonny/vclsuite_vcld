...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt vclutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(vclutil.Amount(5 * vclutil.SatoshiPerBitcoin))
	...
	txid = genSpend(vclutil.Amount(500 * vclutil.SatoshiPerBitcoin))
	blockHashes, err = r.Client.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(vclutil.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*vclutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, vclutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := vclutil.Amount(250 * vclutil.SatoshiPerBitcoin)
	...
	// chain.
	expectedBalance = vclutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := vclutil.Amount(50 * vclutil.SatoshiPerBitcoin)
...
func TestHarness(t *testing.T) {
	...
	// outputs.
	expectedBalance := vclutil.Amount(numMatureOutputs * 50 * vclutil.SatoshiPerBitcoin)
