...
type memWallet struct {
	...
	addrs map[uint32]vclutil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	addrs := make(map[uint32]vclutil.Address)
...
func (m *memWallet) IngestBlock(height int32, header *wire.BlockHeader, filteredTxns []*vclutil.Tx) {
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash,
	...
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          vclutil.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (vclutil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []vclutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt vclutil.Amount,
	feeRate vclutil.Amount, change bool) error {
	...
	for outPoint, utxo := range m.utxos {
		...
		reqFee := vclutil.Amount(txSize * int(feeRate))
...
func (m *memWallet) SendOutputs(outputs []*wire.TxOut,
	feeRate vclutil.Amount) (*chainhash.Hash, error) {
...
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut,
	feeRate vclutil.Amount, change bool) (*wire.MsgTx, error) {
	...
	var outputAmt vclutil.Amount
	for _, output := range outputs {
		outputAmt += vclutil.Amount(output.Value)
		tx.AddTxOut(output)
	}
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() vclutil.Amount {
	m.RLock()
	defer m.RUnlock()
...
func keyToAddr(key *btcec.PrivateKey, net *chaincfg.Params) (vclutil.Address, error) {
	serializedKey := key.PubKey().SerializeCompressed()
	pubKeyAddr, err := vclutil.NewAddressPubKey(serializedKey, net)
	if err != nil {
		return nil, err
	}
